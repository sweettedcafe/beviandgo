-- =====================================================================
-- BEVI & GO — Phase 7 hotfix
-- Fixes: pos_create_order wrote to non-existent public.payments table.
-- Correct table is public.order_payments (Phase 2). Idempotent.
...
begin
  ...
  -- payments (writes to public.order_payments — the actual table from Phase 2)
  for v_payment in select * from jsonb_array_elements(coalesce(p_payload->'payments','[]'::jsonb)) loop
    ...
    v_fee := round(coalesce((v_payment->>'amount')::numeric,0) * v_pm.fee_percent / 100.0, 2) + coalesce(v_pm.fee_fixed,0);
